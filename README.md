...
- Install reddittui from its release binary (see [reddittui](#reddittui))
...
repy uses the same `configuration.json` filename and the same `Setting` /
`Keymap` split as epy, but with snake_case keys — so an epy config does
nothing for it. Both structs are `#[serde(default)]`, so a partial file is
valid and only the differences need writing.

repy's stock keymap already matches the epy config that used to live here:
`j`/`k` scroll, `h`/`l` page, `H`/`L` chapter, and the same bindings for every
action epy had. Writing the rest out would freeze values that are already
correct and drift as repy adds settings. Four values differ:

| Key | Why |
|---|---|
| `dictionary_client: macdict` | repy defaults to `auto` |
| `preferred_tts_engine: null` | repy defaults to `purr` |
| `library: R` | repy defaults to lowercase `r` |
| `add_highlight_comment: C` | repy's own defaults bind this **and** `switch_color` to `c`; highlights are new in repy, so `switch_color` keeps the key it has always had |

**Link the config before launching repy for the first time.** When the file is
absent repy writes a full default config there, which leaves a real file where
the symlink belongs.
...
The two live options differ on one point that decided it: **reddittui scrapes
`old.reddit.com` and needs no credentials**, while `reddix` — the one in
homebrew-core — requires registering a Reddit "script" app and completing an
OAuth flow before it shows a single post. A dotfiles repo that has to carry
API credentials to read a website is the wrong trade.

If reddit ever starts refusing the scrapes, reddittui also speaks to
[Redlib](https://github.com/redlib-org/redlib) instances; the fallback is two
lines in `reddittui/reddittui.toml`, already written out in a comment there.
